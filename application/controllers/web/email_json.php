...
class email_json extends CI_Controller
{
    ...
    function json()
    {
        ...
        $statement = " AND (UPPER(SUBJECT) LIKE '%" . strtoupper($_GET['sSearch']) . "%')";
        $allRecord = $email->getCountByParams(array(), $statement_privacy . $statement);
        // echo $allRecord;exit;
        if ($_GET['sSearch'] == "")
            $allRecordFilter = $allRecord;
        else
            $allRecordFilter =  $email->getCountByParams(array(), $statement_privacy . $statement);

        $email->selectByParams(array(), $dsplyRange, $dsplyStart, $statement_privacy . $statement, $sOrder);
        // echo $email->query;
        // exit;
        ...
        while ($email->nextRow()) {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                if ($aColumns[$i] == "SUBJECT")
                    $row[] = truncate($email->getField($aColumns[$i]), 2);
                else
                    $row[] = $email->getField($aColumns[$i]);
            }
            $output['aaData'][] = $row;
        }
    ...
    function add()
    {
        ...
        $reqBody = $this->input->post("reqBody");
