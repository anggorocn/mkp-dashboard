...
class users_management_json extends CI_Controller
{
    ...
    function json()
    {
        ...
        $aColumns = array(
            "USER_ID", "USERNAME", "FULLNAME", "USER_GROUP",  "BANK_PEMBAYARAN"
                    );

        $aColumnsAlias = array(
            "USER_ID", "USERNAME", "FULLNAME", "USER_LEVEL",  "BANK_PEMBAYARAN"
                    );
        ...
        if (isset($_GET['iSortCol_0'])) {
            ...
            if (trim($sOrder) == "ORDER BY A.USER_ID asc") {
                /*
        ...
        $statement_privacy = " AND A.USER_LEVEL IN (2,3) ";

        $statement = " AND (UPPER(A.FULLNAME) LIKE '%" . strtoupper($_GET['sSearch']) . "%')";
        ...
        while ($usersManagement->nextRow()) {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                ...
                } elseif ($aColumns[$i] == "TRANSAKSI" || $aColumns[$i] == "PELUNASAN" || $aColumns[$i] == "LOCKING_PIUTANG" || $aColumns[$i] == "REPORT" || $aColumns[$i] == "DASHBOARD" || $aColumns[$i] == "MASTER_DATA") {
                    $checked = '';
    ...
    function add()
    {
        ...
        $reqBankPembayaran = $this->db->query(" select nama from bank_pembayaran where bank_pembayaran_id = '$reqBankPembayaranId' ")->row()->NAMA;
        $reqUserLevel = $this->input->post("reqUserLevel");
        $reqMenuMarketing = $this->input->post("reqMenuMarketing");
        $reqMenuFinance = $this->input->post("reqMenuFinance");
        $reqMenuProduction = $this->input->post("reqMenuProduction");
        $reqMenuDocument = $this->input->post("reqMenuDocument");
        $reqMenuSearch = $this->input->post("reqMenuSearch");
        $reqMenuOthers = $this->input->post("reqMenuOthers");
        ...
        if($reqUserLevel == "2" && $reqBankPembayaranId == "")
        {

            $arrResult["status"] = "failed";
            $arrResult["message"] = "Masukkan kode bank terlebih dahulu.";
        ...
        $usersManagement->setField("USER_ID", $reqId);
        $usersManagement->setField("USERNAME", $reqUsername);
        $usersManagement->setField("FULLNAME", $reqFullName);
        $usersManagement->setField("BANK_PEMBAYARAN_ID", $reqBankPembayaranId);
        $usersManagement->setField("BANK_PEMBAYARAN", $reqBankPembayaran);
        $usersManagement->setField("USER_LEVEL", $reqUserLevel);
        $usersManagement->setField("TRANSAKSI", $reqMenuMarketing);
        $usersManagement->setField("PELUNASAN", $reqMenuFinance);
        $usersManagement->setField("LOCKING_PIUTANG", $reqMenuProduction);
        $usersManagement->setField("REPORT", $reqMenuDocument);
        $usersManagement->setField("DASHBOARD", $reqMenuSearch);
        $usersManagement->setField("MASTER_DATA", $reqMenuOthers);
