...
class document_attacment_json extends CI_Controller
{
    ...
    function json()
    {
        ...
        $aColumns = array("DOCUMENT_ATTACMENT_ID","NAME","DESCIPTION","PATH","EXTENSION","SIZE","PRIVIEW","AKSI");
        $aColumnsAlias = array("DOCUMENT_ATTACMENT_ID","NAME","DESCIPTION","PATH","EXTENSION","SIZE","PRIVIEW","AKSI");
        /*
		 * Ordering
		 */
 ...
 $nom=0;
        while ($document_attacment->nextRow()) {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                if ($aColumns[$i] == "NO_PROJECT") {
                    ...
                    $link= "'uploads/eccommerce/".$ids."/".$document_attacment->getField("PATH")."'";
    ...
    function add(){
        ...
        $document_attacment->setField("DOCUMENT_ATTACMENT_ID", $reqId);
    ...
    function add_path_lampiran(){
        ...
        for($i=0;$i<count($reqId);$i++){
            if(!empty($reqId[$i])){
                if($i==0){
                     $statement .= " AND A.DOCUMENT_ATTACMENT_ID='".$reqId[$i]."'";
                }else{
                    $statement .= "  OR A.DOCUMENT_ATTACMENT_ID='".$reqId[$i]."'";
                }
            }
        }
         ...
         $str ='';
         while ($document_attacment->nextRow()) {
            $link = "'uploads/eccommerce/".$document_attacment->getField('DOCUMENT_ATTACMENT_ID')."/".$document_attacment->getField("PATH")."'";
