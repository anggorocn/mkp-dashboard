...
class pembatalan_nota_json extends CI_Controller
{
	...
	function json()
	{
		$this->load->model("PembatalanNota");
		$pembatalan_nota = new PembatalanNota();

		$aColumns = array(
			"PEMBATALAN_NOTA_ID", "POSTING", "NOMOR", "TANGGAL", "NO_NOTA", "TAGIHAN_PROJECT", "NO_FAKTUR", "POSTING_DESC"
		...
		if (isset($_GET['iSortCol_0'])) {
			...
			if (trim($sOrder) == "ORDER BY PEMBATALAN_NOTA_ID asc") {
				/*
				...
				*/
				$sOrder = " ORDER BY PEMBATALAN_NOTA_ID desc";
			}
		}
		...
		$allRecord = $pembatalan_nota->getCountByParamsMonitoring(array(), $statement_privacy . $statement);
		// echo $allRecord;exit;
		if ($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else
			$allRecordFilter =  $pembatalan_nota->getCountByParamsMonitoring(array(), $statement_privacy . $statement);

		$pembatalan_nota->selectByParamsMonitoring(array(), $dsplyRange, $dsplyStart, $statement_privacy . $statement, $sOrder);
		// echo $pembatalan_nota->query;
		// exit;
		...
		while ($pembatalan_nota->nextRow()) {
			$row = array();
			for ($i = 0; $i < count($aColumns); $i++) {
				if ($aColumns[$i] == "USERNAME")
					$row[] = truncate($pembatalan_nota->getField($aColumns[$i]), 2);
				else
					$row[] = $pembatalan_nota->getField($aColumns[$i]);
			}
			$output['aaData'][] = $row;
		}
	...
	function json_posting()
	{
		...
		$aColumns = array(
			"PEMBATALAN_NOTA_ID", "POSTING", "NOMOR", "TANGGAL", "NO_NOTA", "TAGIHAN_PROJECT", "NO_FAKTUR"
	...
	function add()
	{
		...
		$pembatalan_nota->setField("PEMBATALAN_NOTA_ID", $reqId);
		$pembatalan_nota->setField("ALASAN_BATAL", $reqAlasan);
		$pembatalan_nota->setField("TAGIHAN_PROJECT_ID", $reqTagihanId);
		$pembatalan_nota->setField("PEGAWAI_ID_APPROVAL1", $reqPegawaiApproval1);
		$pembatalan_nota->setField("PEGAWAI_ID_APPROVAL2", $reqPegawaiApproval2);
        ...
        for ($i=0; $i < count($reqLinkFile['name']); $i++) { 
            $renameFile = "PEMBATALAN" . date("dmYhis") . rand() . "." . getExtension($reqLinkFile['name'][$i]);
        ...
        $pembatalan_nota->setField("LAMPIRAN", implode(";", $insertLinkFile));


		if (empty($reqId)) {
			$pembatalan_nota->setField("CREATED_BY", $this->USERNAME);
			$pembatalan_nota->setField("CREATED_DATE", "CURRENT_DATE");
			$pembatalan_nota->insert();
			$reqId = $pembatalan_nota->id;
		} else {
			$pembatalan_nota->setField("UPDATED_BY", $this->USERNAME);
			$pembatalan_nota->setField("UPDATED_DATE", "CURRENT_DATE");
			$pembatalan_nota->update();
		}
    ...
    function posting()
    {
        ...
        $query = $this->db->query(" SELECT NOMOR, POSTING, PEGAWAI_ID_APPROVAL1, PEGAWAI_ID_APPROVAL2 FROM PEMBATALAN_NOTA X WHERE X.PEMBATALAN_NOTA_ID = '".$reqId."'  ");
        ...
        $statusEmail1 = $userMobile->pushNotifikasi($reqPegawaiApproval1, $reqPegawaiId, $reqNama, $reqId, $reqNomor, "PEMBATALAN NOTA", "PEMBATALAN_NOTA", "APPROVAL1", 1);
    

        if(!empty($reqPegawaiApproval2))
        {
            $statusEmail2 = $userMobile->pushNotifikasi($reqPegawaiApproval2, $reqPegawaiId, $reqNama, $reqId, $reqNomor, "PEMBATALAN NOTA", "PEMBATALAN_NOTA", "APPROVAL2", 2, false);
        }
        ...
        $this->db->query(" UPDATE PEMBATALAN_NOTA X SET POSTING = 'POSTING', POSTING_DATE = SYSDATE WHERE X.PEMBATALAN_NOTA_ID = '".$reqId."' "); 
