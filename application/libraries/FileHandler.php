...
class FileHandler {
	...
	**/
	...
	function cekSize($filename,$tempName){
		$arrData = array();
		$fileLinkSize = $filename['size'];
		$fileLinkName = $filename['name'];
		$str ='';
		$nomer =1;
		for($i=0;$i<count($tempName);$i++){
			if(!empty($fileLinkName[$i])){
			// $str .= '[ '.$nomer.' ] Nama Files '.$fileLinkName[$i].' File Size '.$this->convert_bytes_to_hr_format($fileLinkSize[$i]).'<br>';
			if($fileLinkSize[$i]>19999999){
					$str .='[ '.$nomer.' ]  Nama Files '.$fileLinkName[$i].' File Size '.$this->convert_bytes_to_hr_format($fileLinkSize[$i]).'<br>';
		...
		if(!empty($str)){
			echo 'xxx-'.$str.'<br> <b>Out Of Size More 20 MB</b>';exit;
			// echo 'xxx-'.$str;
			return false;
		}else{
			return true;
