...
class Document extends Entity
{
    ...
    function insert()
    {
        ...
        $this->setField("DOCUMENT_ID", $this->getSeqId("DOCUMENT_ID", "DOCUMENT"));
    ...
    {
        $str = " SELECT COUNT(*) AS ROWCOUNT FROM DOCUMENT WHERE CATEGORY = 'Legality Letters' AND (EXPIRED_DATE < NOW() OR (NOW() BETWEEN (EXPIRED_DATE + INTERVAL '14 DAY') AND (EXPIRED_DATE + INTERVAL '1 DAY'))); " . $statement;
        while (list($key, $val) = each($paramsArray)) {
            $str .= " AND $key =    '$val' ";
        }
        $this->query = $str;
