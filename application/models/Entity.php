  ...
  ***/
  ...
  class Entity extends CI_Model{ 
    ...
    **/
	...
	public function getBearerToken($idPegawai,$filter=""){
		$str = "SELECT SHA256.ENCRYPT(MD5('".$idPegawai."'||'F1N4T3LukL4M0n6'||TO_CHAR(CURRENT_TIMESTAMP, 'HH24:MI'))) AS JUMLAH FROM DUAL    ";
	...
	public function getToken($idPegawai,$filter=""){
		$str = "SELECT MD5('".$idPegawai."'||'F1N4T3LukL4M0n6'||TO_CHAR(CURRENT_TIMESTAMP, 'HH24:MI')) AS JUMLAH FROM DUAL    ";
			$query = $this->db->query($str);
	...
	public function getNextId($idName,$tableName,$filter=""){
		$str = "SELECT MAX(TO_NUMBER($idName)) HASIL FROM $tableName ";
		if($filter != "")
	...
	public function getSeqId($idName,$tableName,$filter=""){
		$str = "SELECT SEQ_".$tableName.".NEXTVAL HASIL FROM DUAL ";
		$rowResult = $this->db->query($str)->row();
		return $rowResult->HASIL;
	}
	...
	function selectLimit2($sql,$numrow=-1,$offset=-1){
