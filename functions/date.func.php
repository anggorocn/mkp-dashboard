...
function dateToPageCheck3($_date)
{
	if ($_date == "") {
		return "";
	}
...
function dateToDBCheck2($_date)
{
	...
	$arrDate = explode("/", $_date);
	$_date = $arrDate[2] . "-" . generateZeroDate($arrDate[0], 2) . "-" . generateZeroDate($arrDate[1], 2);
	return "TO_DATE('" . $_date . "', 'YYYY-MM-DD')";
}
...
function getTglBlnTahun($date)
{
	$val = explode('-', $date);
	$vals = $val[0] . ' ' . getExtMonth((int)$val[1]) . ' ' . $val[2];
	return $vals;
}
...
function getFormattedDateViewNew($_date)
{
	$arrMonth = array(
		"1" => "Januari", "2" => "Februari", "3" => "Maret", "4" => "April", "5" => "Mei",
		"6" => "Juni", "7" => "Juli", "8" => "Agustus", "9" => "September", "10" => "Oktober",
	...
	$date = '' . $arrMonth[$_month] . ' ' . $arrDate[0]  . ', ' . $arrDate[2] . '';
	return $date;
}
// date input : database
