...
function getNamePeriode(vperiode)
{
	
	bulan = vperiode.substring(0, 2);
	tahun = vperiode.substring(2, 6);

	return getNameMonth(bulan) + " " + tahun;
}
...
function FormatUang(id)
{
	...
	var hasil =(formatted + ((parts) ? "," + parts[1].substr(0, 4) : "")); 

	adaMinus  = hasil.indexOf('-.');
...
function FormatUangNoMinus(id)
{
	...
	formatted = output.reverse().join("");
	var hasil =(formatted + ((parts) ? "," + parts[1].substr(0, 3) : "")); 
...
function konfirmasiDeleteData(delele_link, id, csrf)
{
	...
	$.messager.confirm('Konfirmasi','Yakin menghapus data terpilih ?',function(r){
			if (r){
				...
				var win = $.messager.progress({
					title:'Aplikasi Rombongan | Ancol Korporat',
					msg:'proses...'
				});				

				$.post( delele_link, { reqId: id, '_token': csrf })
				  .done(function( data ) {
...
function konfirmasiAksiData(pesan, link, id, csrf)
{
	...
	$.messager.confirm('Konfirmasi', pesan,function(r){
			if (r){
				...
				$.post( link, { reqId: id, '_token': csrf })
				  .done(function( data ) {
					$.messager.progress('close');
...
function konfirmasiJSON(konfirmasi, web_link)
{
	$.messager.confirm('Konfirmasi',konfirmasi,function(r){
			if (r){
