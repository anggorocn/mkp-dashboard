...
function(t, e) {
	...
	return i(e, "Core/Globals.js", [], function() {
		var e, i;
		return (i = e || (e = {})).SVG_NS = "http://www.w3.org/2000/svg", i.product = "Highcharts", i.version = "11.4.4", i.win = void 0 !== t ? t : {}, i.doc = i.win.document, i.svg = i.doc && i.doc.createElementNS && !!i.doc.createElementNS(i.SVG_NS, "svg").createSVGRect, i.userAgent = i.win.navigator && i.win.navigator.userAgent || "", i.isChrome = i.win.chrome, i.isFirefox = -1 !== i.userAgent.indexOf("Firefox"), i.isMS = /(edge|msie|trident)/i.test(i.userAgent) && !i.win.opera, i.isSafari = !i.isChrome && -1 !== i.userAgent.indexOf("Safari"), i.isTouchDevice = /(Mobile|Android|Windows Phone)/.test(i.userAgent), i.isWebKit = -1 !== i.userAgent.indexOf("AppleWebKit"), i.deg2rad = 2 * Math.PI / 360, i.hasBidiBug = i.isFirefox && 4 > parseInt(i.userAgent.split("Firefox/")[1], 10), i.marginNames = ["plotTop", "marginRight", "marginBottom", "plotLeft"], i.noop = function() {}, i.supportsPassiveEvents = function() {
			let t = !1;
			if (!i.isMS) {
 ...
 - ${i}: ${e}`, h && (l += encodeURI(i) + "=" + encodeURI(e))
		...
		class I {
			...
			getBBox(t, e) {
				...
				let {
					...
					cache: c,
					cacheKeys: p
				} = h, u = a.namespaceURI === this.SVG_NS, g = T(e, this.rotation, 0), f = h.styledMode ? a && I.prototype.getStyle.call(a, "font-size") : l.fontSize;
			...
			getRotatedBox(t, e) {
				let {
					...
					[S, C],
					[k, M]
				] = [l, d].map(t => [t - t * f, t * m]), w = i + c * (r - x) + S + M + p * b, A = w + x, T = A - o * b, P = T - x, L = s + p - c * y - C + k + p * v, O = L + y, D = O - o * v, E = D - y, I = Math.min(w, A, T, P), j = Math.min(L, O, D, E), B = Math.max(w, A, T, P) - I, R = Math.max(L, O, D, E) - j;
				return {
					x: I,
					y: j,
					width: B,
			...
			symbol(t, e, i, s, r, o) {
				...
				if (m) "number" == typeof e && (a = m.call(this.symbols, e || 0, i || 0, s || 0, r || 0, o)), n = this.path(a), d.styledMode || n.attr("fill", "none"), L(n, {
					...
					s.imgwidth = B(o && o.width, v[h] && v[h].width), s.imgheight = B(o && o.height, v[h] && v[h].height), l = t => t.attr({
						...
						x: e,
						y: i
					}), s.isImg = !0, T(s.imgwidth) && T(s.imgheight) ? l(s) : (s.attr({
						width: 0,
						height: 0
					}), M("img", {
						onload: function() {
		...
		class N {
					...
					e.hasVisibleSeries = !1, e.dataMin = e.dataMax = e.threshold = void 0, e.softThreshold = !e.isXAxis, e.series.forEach(i => {
						if (i.reserveSpace()) {
							...
							if (e.hasVisibleSeries = !0, e.positiveValuesOnly && 0 >= (o || 0) && (o = void 0), e.isXAxis)(r = i.xData) && r.length && (r = e.logarithmic ? r.filter(t => t > 0) : r, n = (t = i.getXExtremes(r)).min, a = t.max, T(n) || n instanceof Date || (r = r.filter(T), n = (t = i.getXExtremes(r)).min, a = t.max), r.length && (e.dataMin = Math.min(E(e.dataMin, n), n), e.dataMax = Math.max(E(e.dataMax, a), a)));
								...
								T(t.dataMin) && (n = t.dataMin, e.dataMin = Math.min(E(e.dataMin, n), n)), T(t.dataMax) && (a = t.dataMax, e.dataMax = Math.max(E(e.dataMax, a), a)), y(o) && (e.threshold = o), (!s.softThreshold || e.positiveValuesOnly) && (e.softThreshold = !1)
							}
						}
					})
			...
			getPlotLinePath(t) {
				...
				return k(this, "getPlotLinePath", v, function(t) {
					u = f = (c = m(c = E(c, e.translate(n, void 0, void 0, o)), -1e5, 1e5)) + d, g = x = h - c - d, T(c) ? e.horiz ? (g = r, x = h - e.bottom + (i.scrollablePixelsY || 0), u = f = b(u, s, s + e.width)) : (u = s, f = l - e.right + (i.scrollablePixelsX || 0), g = x = b(g, r, r + e.height)) : (y = !0, p = !1), t.path = y && !p ? void 0 : i.renderer.crispLine([
						["M", u, g],
						["L", f, x]
					], a || 1)
				}), v.path
			...
			adjustForMinRange() {
				let t = this.options,
					...
					o, n, a, h;
				this.isXAxis && void 0 === r && !e && (r = y(t.min) || y(t.max) || y(t.floor) || y(t.ceiling) ? null : Math.min(5 * (M(this.series.map(t => (t.xIncrement ? t.xData?.slice(0, 2) : t.xData) || [])) || 0), this.dataMax - this.dataMin)), T(i) && T(s) && T(r) && i - s < r && (n = this.dataMax - this.dataMin >= r, o = (r - i + s) / 2, a = [s - o, E(t.min, s - o)], n && (a[2] = e ? e.log2lin(this.dataMin) : this.dataMin), h = [(s = g(a)) + r, E(t.max, s + r)], n && (h[2] = e ? e.log2lin(this.dataMax) : this.dataMax), (i = f(h)) - s < r && (a[0] = i - r, a[1] = E(t.min, i - r), s = g(a))), this.minRange = r, this.min = s, this.max = i
			}
			getClosest() {
				let t, e;
			...
			setTickInterval(t) {
				let {
					...
					softMax: f,
					softMin: m
				} = h, b = T(g) && i[this.coll]?.[g], v = h.tickPixelInterval, C = h.maxPadding, M = h.minPadding, w = 0, A, P = T(h.tickInterval) && h.tickInterval >= 0 ? h.tickInterval : void 0, L, O, D, I;
			...
			setTickPositions() {
				...
				this.tickPositions = a, this.paddedTicks = a.slice(0), this.trimTicks(a, o, n), !this.isLinked && T(this.min) && T(this.max) && (this.single && a.length < 2 && !this.categories && !this.series.some(t => t.is("heatmap") && "between" === t.options.pointPlacement) && (this.min -= .5, this.max += .5), e || h || this.adjustTickAmount()), k(this, "afterSetTickPositions")
			}
			trimTicks(t, e, i) {
				let s = t[0],
					r = t[t.length - 1],
		...
		class c {
			...
			render() {
				...
				let {
					...
					label: b,
					svgElem: v
				} = this, S = [], C, k = r(f) && r(g), M = r(m), w = !v, A = {
					class: "highcharts-plot-" + (k ? "band " : "line ") + (e.className || "")
				}, T = k ? "bands" : "lines";
				if (!t.chart.styledMode && (M ? (A.stroke = o || "#999999", A["stroke-width"] = d(e.width, 1), e.dashStyle && (A.dashstyle = e.dashStyle)) : k && (A.fill = o || "#e6e9ff", x && (A.stroke = e.borderColor, A["stroke-width"] = x))), p.zIndex = c, T += "-" + c, (C = t.plotLinesAndBandsGroups[T]) || (t.plotLinesAndBandsGroups[T] = C = u.g("plot-" + T).attr(p).add()), v || (this.svgElem = v = u.path().attr(A).add(C)), r(m)) S = t.getPlotLinePath({
					value: s?.log2lin(m) ?? m,
				...
				return !this.eventsAdded && n && (l(n, (t, e) => {
					...
					d: S
				})) : v.d && (v.hide(), b && (this.label = b = b.destroy()))), y && (r(y.text) || r(y.formatter)) && S?.length && t.width > 0 && t.height > 0 && !S.isFlat ? (y = h({
					align: i && k ? "center" : void 0,
					x: i ? !k && 4 : 10,
					verticalAlign: !i && k ? "middle" : void 0,
					y: i ? k ? 16 : 10 : k ? 6 : -4,
					rotation: i && !k ? 90 : 0
				}, y), this.renderLabel(y, S, k, c)) : b && b.hide(), this
			}
			renderLabel(t, e, r, o) {
				let n = this.axis,
					a = n.chart.renderer,
		...
		class T {
			...
			getPosition(t, e, i) {
				...
				let A = !this.followPointer && k(i.ttBelow, !d && !a === w),
					...
					O = () => {
						!1 !== T.apply(0, S) ? !1 !== P.apply(0, C) || M || (L(!0), O()) : M ? u.x = u.y = 0 : (L(!0), O())
					};
			...
			refresh(t, e) {
				let {
					...
					pointer: r,
					shared: n
				} = this, a = w(t), l = a[0], d = [], c = s.format, p = s.formatter || this.defaultFormatter, u = i.styledMode, g = {};
				if (!s.enabled || !l.series) return;
				o.clearTimeout(this.hideTimer), this.allowShared = !(!b(t) && t.series && t.series.noSharedTooltip), this.followPointer = !this.split && l.series.tooltipOptions.followPointer;
				...
				let v = S(c) ? h(c, g, i) : p.call(g, this),
					C = l.series;
				if (this.distance = k(C.tooltipOptions.distance, 16), !1 === v) this.hide();
				else {
		...
		class S {
			...
			destroy() {
				let t = this;
				this.eventsToUnbind.forEach(t => t()), this.eventsToUnbind = [], !e.chartCount && (S.unbindDocumentMouseUp && (S.unbindDocumentMouseUp = S.unbindDocumentMouseUp()), S.unbindDocumentTouchEnd && (S.unbindDocumentTouchEnd = S.unbindDocumentTouchEnd())), clearInterval(t.tooltipTimeout), m(t, function(e, i) {
					t[i] = void 0
				})
			}
		...
		class X {
			...
			setData(t, e = !0, i, s) {
				...
				if (l && l.enabled && (t = this.sortData(t)), h.options.chart.allowMutatingData && !1 !== s && k && n && !r.cropped && !r.hasGroupedData && r.visible && !r.boosted && (b = this.updateData(t, i)), !b) {
					...
					for (u && B(u[0]) && A(14, !0, h), r.data = [], r.options.data = r.userOptions.data = t, x = n; x--;) o[x]?.destroy();
		...
		class Q {
			...
			getContainer() {
				let t = this.options,
					...
					s = J(),
					r, a = this.renderTo;
				a || (this.renderTo = a = e.renderTo), F(a) && (this.renderTo = a = C.getElementById(a)), a || B(13, !0, this);
				let h = $(P(a, i));
				H(h) && S[h] && S[h].hasRendered && S[h].destroy(), P(a, i, this.index), a.innerHTML = p.emptyHTML, e.skipClone || a.offsetWidth || this.temporaryDisplay(), this.getChartSize();
				...
				if (this.renderer = new g(u, c, d, void 0, e.forExport, t.exporting && t.exporting.allowHTML, this.styledMode), m(void 0, this), this.setClassName(e.className), this.styledMode)
			...
			transform(t) {
				...
				for (let t of (this.hoverPoints?.forEach(t => t.setState()), e)) {
					...
					let M = t.toValue(C, !0) + (o ? 0 : u * k),
						...
						A = t.allExtremes;
					if (M > w && ([M, w] = [w, M]), 1 === v && !r && "yAxis" === t.coll && !A) {
						for (let e of t.series) {
							let t = e.getExtremes(e.getProcessedData(!0).yData, !0);
							A ?? (A = {
								dataMin: Number.MAX_VALUE,
								dataMax: -Number.MAX_VALUE
							}), H(t.dataMin) && H(t.dataMax) && (A.dataMin = Math.min(t.dataMin, A.dataMin), A.dataMax = Math.max(t.dataMax, A.dataMax))
						}
						t.allExtremes = A
					}
					let {
						...
						min: L,
						max: O
					} = R(t.getExtremes(), A || {}), E = T ?? g.min, I = P ?? g.max, j = w - M, B = t.categories ? 0 : Math.min(j, I - E), z = E - B * (D(g.min) ? 0 : g.minPadding), N = I + B * (D(g.max) ? 0 : g.maxPadding), W = t.allowZoomOutside || 1 === v || "zoom" !== a && v > 1, G = Math.min(g.min ?? z, z, W ? L : z), X = Math.max(g.max ?? N, N, W ? O : N);
					(!t.isOrdinal || 1 !== v || r) && (M < G && (M = G, v >= 1 && (w = M + j)), w > X && (w = X, v >= 1 && (M = w - j)), (r || t.series.length && (M !== L || w !== O) && M >= G && w <= X) && (o ? o[t.coll].push({
						axis: t,
						min: M,
						max: w
		...
		function M(t, e) {
			...
			for (P.stacksTouched += 1, c = 0; c < v; c++) {
				...
				let e = o.total || 0;
				"percent" === f ? (n = r ? w : A, e = T && L[n]?.[p] ? (n = L[n][p]).total = Math.max(n.total || 0, e) + Math.abs(g) || 0 : l(e + (Math.abs(g) || 0))) : "group" === f ? (u(g) && (g = g[0]), null !== g && e++) : e = l(e + (g || 0)), "group" === f ? o.cumulative = (e || 1) - 1 : o.cumulative = l(m(o.cumulative, k) + (g || 0)), o.total = e, null !== g && (o.points[h].push(o.cumulative), b[c] = o.cumulative, o.hasValidPoints = !0)
			}
					...
					l && (t.sortByAngle(e, n - .5), C > 0 && (f = Math.max(0, g - c - C), m = Math.min(g + c + C, i.plotHeight), e.forEach(t => {
						...
						(i.dataLabels || []).forEach(h => {
							let l = h.options || {},
								...
								A = "inherit";
							if (f) {
								if (d && p(g) && f.distance > 0 && (void 0 === g.pos ? A = "hidden" : (S = g.size, w = y.radialDistributionY(i, h))), l.justify) M = y.justify(i, h, c, a);
